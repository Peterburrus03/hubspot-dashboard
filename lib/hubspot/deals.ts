...
function transformDeal(hubspotDeal: HubSpotDeal, stageMap: Map<string, string>, companyId?: string, contactId?: string): Deal {
  ...
  const stageEnteredDate = hsDateEntered ?? (() => {
    switch (stageLabel2) {
      ...
      case 'Data Collection (including NDA)':  return parseHubSpotDate(props.nda_signed_date)
      case 'Pre-LOI Analysis':                 return parseHubSpotDate(props.nda_signed_date)
  ...
  return {
    ...
    ndaSignedDate: parseHubSpotDate(props.nda_signed_date),
