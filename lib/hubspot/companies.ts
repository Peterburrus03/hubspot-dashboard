...
async function fetchAllCompaniesFromHubSpot(): Promise<HubSpotCompany[]> {
  ...
  console.log(`Fetched ${results.length} companies from HubSpot`)
  return results
}
...
export async function getCompanies(forceRefresh = false): Promise<Company[]> {
  ...
  // Fetch all companies that have a practice_type set in HubSpot
  const hubspotCompanies = await fetchAllCompaniesFromHubSpot()
