...
export async function GET() {
  try {
    ...
    // Outreach = non-automated emails + calls + notes + meetings
    ...
    const wtdEnd = new Date(wtdStart.getTime() + 7 * 24 * 60 * 60 * 1000)
    ...
    const weeklyOutreach = await prisma.$queryRaw<WeekRow[]>(Prisma.sql`
      ...
      WHERE timestamp >= ${historyStart}
        AND timestamp < ${wtdEnd}
      GROUP BY week_start
      ORDER BY week_start
    `)

    // Weekly NDA history — cap at current week
