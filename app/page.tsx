...
export default function OverviewPage() {
  const [selectedQ, setSelectedQ] = useState<Quarter>(getCurrentQuarter)
  const [data, setData] = useState<any>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    setLoading(true)
    fetch(`/api/dashboard/overview?q=${selectedQ}`)
      .then(r => r.json())
      .then(d => {
        setData(d)
        setLoading(false)
  ...
  const currentStatus = realWeeks[currentWeekIdx] ?? { cumulativeDelta: 0 }
  const qLabel = `Q${selectedQ}`
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tight uppercase">Outreach Performance</h2>
      ...
      {loading ? (
        <div className="p-8 text-center text-gray-500 animate-pulse font-bold uppercase tracking-widest">
          Loading {qLabel} Performance...
        </div>
      ) : (
        <>
          {/* KPI Row */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label={`${qLabel} Total Goal`} value={data.qTotalTarget} sublabel="Total touches target" color="gray" />
            <StatCard label="Actual to Date" value={data.qTotalActual} sublabel={`${((data.qTotalActual / data.qTotalTarget) * 100).toFixed(1)}% of ${qLabel} complete`} color="blue" />
            <StatCard
              label="Current Delta"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-black text-gray-900 uppercase tracking-tight text-sm">{qLabel} Cumulative Progress</h3>
          ...
          <Card padding="none">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <h3 className="font-black text-gray-900 uppercase tracking-tight text-sm">{qLabel} Marketing Outreach Ledger</h3>
